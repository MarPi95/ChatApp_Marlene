...
class User(db.Model):
    ...
    messages = db.relationship('Message', backref='user', lazy=True) #one to many, 1 user mehrere Messages, lazyloading = erst wenn geladen und verwendet dann wird es geladen
...
def logout():
    ...
    response = make_response(render_template("index.html", error="User is not logged in."))
    if session_token:
...
def delete():
    ...
    if message_id in user_model.messages:
